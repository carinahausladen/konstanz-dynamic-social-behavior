...
One reason the first two papers this week feel like a break from the earlier LLM-behaviour literature is that they no longer trust the language model, by prompting alone, to carry the full strategic burden. In the Week-2 through Week-5 papers, the recurring question was whether an LLM could itself behave strategically, cooperatively, or believably under the right framing or architecture. In `Xu` and `FAIR`, the design stance is different. Strategy is pulled *out* of the language model and assigned to an explicit optimiser: in `Xu`, the LLM proposes candidate moves and an RL component helps choose among them; in `FAIR`, a planner selects the action and intent first and the language model then realises that plan in speech. Week 6 is therefore the first point in the course where the papers treat the LLM less as the strategic agent itself and more as one component inside a larger strategic architecture.
...
The reason a small theoretical paper about Q-learning in the Prisoner's Dilemma ended up in *Games and Economic Behavior* is that it speaks directly to one of the most active questions in current competition policy: **can pricing algorithms tacitly collude on supra-competitive prices without any agreement between the firms that deployed them, simply through their joint learning dynamics?** Three results make this a live worry. `Calvano, Calzolari, Denicolò & Pastorello (2020, AER)` show by simulation that two Q-learning pricing bots in a Bertrand competition game converge to supra-competitive prices — *de facto* collusion — without any communication, using Q-learners with state. `Assad, Clark, Ershov & Xu (2022, JPE)` show empirically that the adoption of pricing algorithms in the German retail gasoline market raised margins, with effects concentrated in stations whose competitors had also adopted algorithms. `Klein (2021)` characterises when algorithmic collusion is sustainable as a function of the algorithm's parameters.
