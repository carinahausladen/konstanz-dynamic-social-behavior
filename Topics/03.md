...
- `Horton 2023` — Persona prompting as a theory tool.

**🟩 Field maps and methodological frameworks** — *describe or critique the space*

- `Sun 2025` — Four-branch taxonomy of the field.
- `Kozlowski & Evans 2025` — Six-weakness validity framework. Jingyao presents.

**🟨 Static behavioral evaluation** — *run LLM through games, measure aggregate behavior*

- `Lorè & Heydari 2024` — One-shot 2×2 social-dilemma factorial.
- `Fan 2024` 
- `MindGames 2025` — Theory-of-mind + strategic-game benchmark.

**🟥 Dynamic / multi-agent evaluation** — *iterate, see what happens*

- `Akata 2025` — Ten rounds × 1,224 games, seven families.
- `Willis 2025` — LLM-authored strategies compete in a Moran-process tournament.

**🟪 Fine-tuning alternative** — *train a new model rather than prompt an existing one*

- `Binz 2025 — Centaur.` Llama 3.1 70B fine-tuned on 10.7M human choices from 160 psychology experiments.

🐍 **Notebook:** [`3_ipd_tournament.ipynb`](../JN/week3/3_ipd_tournament.ipynb)
...
### One figure worth keeping in view
...
What iteration does *not* do, on its own, is surface within-model heterogeneity. Akata reports model-level strategic profiles. Willis reports model-level evolutionary tendencies. In neither paper does the distribution of *trajectories within a model* become the object of analysis — despite the fact that, with 1,224 games × 10 rounds in Akata's panel, the data to ask the question exist. The red category extends what the field sees along the time axis without extending what it sees along the population axis. Both extensions are available.

---

## 7. 🟩 What Kozlowski and Evans add

`Kozlowski and Evans` approach the same literature from a different angle. Where much of this literature asks whether the behavioral-GT *method* transfers to LLMs, they ask whether the *validity apparatus* that social science developed for human subjects transfers at all. Their answer is a structured list of six reasons to worry: bias, uniformity, atemporality, linguistic cultures, disembodiment, and "alien intelligence."

The uniformity concern in particular — their Figure 6, which shows GPT-4 responding to the General Social Survey with far lower variance than the human population does — names a recurring worry about LLM populations. Low variance within a persona is a *symptom* of the missed-mixture problem — the gap between an LLM's bunched response distribution and the spread of a human population — from the other direction. If GPT-4's response distribution is bunched at one or two modes where the human distribution is spread, then either the model has a few behavioral types that the interview prompt fails to separate, or the model lacks the kind of within-subject variability that makes humans a population rather than a prototype. Either interpretation has consequences for simulation-based research.
